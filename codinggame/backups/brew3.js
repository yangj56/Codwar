...
function transform(brewArr, inventoryArr, spellArr, shortOf) {
    ...
    for (let j = 0; j < brewArr.length; j++) {
        ...
        const shortBlue = Math.abs(selectArr.arr[0]) > 0 && diffinBlue < 0 ? diffinBlue : 0;
        const shortGreen = Math.abs(selectArr.arr[1]) > 0 && diffinGreen < 0 ? diffinGreen : 0;
        const shortOrange = Math.abs(selectArr.arr[2]) > 0 && diffinOrange < 0 ? diffinOrange : 0;
        const shortYellow = Math.abs(selectArr.arr[3]) > 0 && diffinYellow < 0 ? diffinYellow : 0;
...
function canMakeUpTwice(shortArr, spellArr, shortOf, twiceIndex) {
    console.error('twice shortArr');
    console.error(shortArr);
    console.error('twice spellArr');
...
function canMakeUp(shortArr, spellArr, shortOf) {
    ...
    for (let j = 0; j < spellArr.length; j++) {
        ...
        newShortArr[2] += selectArr.arr[2];
        newShortArr[3] += selectArr.arr[3];
        console.error(`${newShortArr[0]} ${newShortArr[1]} ${newShortArr[2]} ${newShortArr[3]}`);
        if (newShortArr[0] >= 0 && newShortArr[1] >= 0 && newShortArr[2] >= 0 && newShortArr[3] >= 0) {
            spellActionIds.push(selectArr);
            console.error(`Solv11111`);
            console.error(selectArr);
            solved = true;
            break;
        }
        spellActionIds.push(selectArr);
